...
我们使用Python实现了GMCP-R协议的原型系统。全部实验在同一主机上通过TCP loopback完成，客户端与服务器分别监听 `127.0.0.1:9000` 与 `127.0.0.1:9001`。

| 组件 | 实现 |
|------|------|
| 运行环境 | Python 3.11, 同一主机, TCP loopback |
| 网络地址 | 127.0.0.1:9000 (服务端), 127.0.0.1:9001 (客户端) |
...
| 重复次数 | 基线30次/配置、票据30次/配置、恢复30次/配置、性能30次/配置、并发30次/并发等级、弱网2次/配置、Checkpoint 90次/配置 |
| 并发客户端数 | 1, 2, 5, 10, 20 |
...
本节系统呈现GMCP-R协议在安全性、性能、可扩展性及弱网适应性四个维度的实验评估结果。各实验重复次数按配置分组（基线对比、票据拒绝、恢复实验各30次/配置，性能基准与并发各30次/配置，弱网仿真2次/配置，Checkpoint成本90次/配置），取均值报告。
...
为量化GMCP-R的O(k)恢复效率相对于Authenticated Hash Chain的O(n)恢复的改进幅度，我们设计了Checkpoint成本对比实验。实验固定n=100,000条消息，变化Checkpoint间隔k∈{10, 50, 100, 500}，每种配置重复90次。从不同偏移量（offset）触发恢复，测量恢复时间、物理读取字节数与重放记录数。实验结果如表7所示。

**表7：Checkpoint成本对比（n=100,000，90次重复均值）**
