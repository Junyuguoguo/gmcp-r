...
def build_document():
    ...
    add_p(
        doc,
        "Python TCP baseline 对比实验覆盖 4 种协议、5 类攻击类型、3 种消息数量、2 种载荷大小与 30 次重复，共 3,600 条记录。MemoryTicket 无效票据实验覆盖 5 类无效票据、2 种消息数量与 30 次重复，共 300 条记录。攻击/断连恢复实验覆盖 5 类场景、3 种消息数量、2 种载荷大小与 30 次重复，共 900 条记录。弱网仿真覆盖 3 种协议、5 档丢包率、5 档延迟与每条件 2 次重复，结果作为初步恢复能力验证，而非操作系统级网络损伤结论。",
    )
    ...
    add_p(
        doc,
        "本节采用与表 6 一致的 v2 汇总口径报告描述性统计。可以看到，GMCP-R 与 Hash Chain 在测试攻击集合中的检测率均为 100%，而 Seq+MAC 和 Ticket Only 分别存在 25% 与 50% 的误接受率。考虑到当前 baseline 每个配置的重复次数仍较少，本稿暂不报告正式统计推断，后续扩展重复次数后再补充显著性检验。",
    )
    add_figure(
        doc,
